...
def test_dataset_single_item_shapes(tmp_path: Path):
    ...
    # Get first sample
    obs, actions, mask, weight = dataset[0]
...
def test_invalid_json(tmp_path: Path):
    ...
    with dataset_path.open("w") as f:
        f.write(
            '{"observation": [1.0, 2.0, 3.0, 4.0], "action": [0, 1], "mask": [[1,1,1,1],[1,1,1,1]]}\n'
        )
        f.write("this is not valid json\n")
        f.write(
            '{"observation": [3.0, 4.0, 5.0, 6.0], "action": [1, 0], "mask": [[1,1,1,1],[1,1,1,1]]}\n'
        )
